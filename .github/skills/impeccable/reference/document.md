...
**The [Rule Name] Rule.** [Short, forceful prohibition or doctrine, e.g. "The One Voice Rule. The primary accent is used on â‰¤10% of any given screen. Its rarity is the point."]
...
- **Body** ([weight], [size], [line-height]): [Purpose. Include max line length like 65â€“75ch if relevant.]
...
- `narrative.northStar` â†’ the `**Creative North Star: "..."**` line from Overview
- `narrative.overview` â†’ the philosophy paragraphs from Overview
- `narrative.keyCharacteristics` â†’ the bulleted `**Key Characteristics:**` list
- `narrative.rules` â†’ every `**The [Name] Rule.** [body]` across all sections, tagged with `section`
- `narrative.dos` / `narrative.donts` â†’ the bullet lists from Do's and Don'ts verbatim
...
1. **Color strategy.** Pick one:
   - Restrained: tinted neutrals + one accent â‰¤10%
   - Committed: one saturated color carries 30â€“60% of the surface
   - Full palette: 3â€“4 named color roles, each deliberate
...
- **Elevation**: inferred from motion energy. Restrained/Responsive â†’ flat by default; Choreographed â†’ layered. One sentence.
...
- **Cite PRODUCT.md anti-references by name** in the Do's and Don'ts section. If PRODUCT.md lists "SaaS landing-page clichÃ©s" or "generic AI tool marketing" as anti-references, the DESIGN.md Don'ts should repeat those phrases verbatim so the visual spec enforces the strategic line.
